...
const paymentMethods = [
  ...
  { id: "Cash on Delivery", label: "Cash", icon: "💵" },
  { id: "Card", label: "Card", icon: "💳" },
];

/* Icon map for categories — add more as needed */
        ...
        return (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-xs mt-1.5 font-medium ${active ? "text-blue-600" : done ? "text-green-500" : "text-gray-400"}`}
              >
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div
                className={`h-0.5 w-16 mb-5 mx-1 transition-all ${current > step ? "bg-green-400" : "bg-gray-200"}`}
              />
            )}
          </React.Fragment>
        );
...
export default function CustomerBookingPage() {
    ...
    if (step === 2 && form.category_id) {
      ...
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          // Round to 4dp — fits DecimalField(max_digits=10, decimal_places=8) safely
          ...
          api
            .get("/api/servicemen/nearby/", {
              params: {
                category: form.category,  // backend reads category name string
                lat: latitude,
                lon: longitude,
              },
            })
            .then((res) => {
              console.log("Nearby workers response:", res.data);
              setWorkers(res.data);
            })
            .catch((err) => {
  ...
  const goNext = () => {
    setError("");
    if (step === 1 && !form.category_id) {
      setError("Please select a service category.");
      return;
    }
    if (step === 2 && !selectedWorker) {
      setError("Please select a professional to continue.");
      return;
    }
    setStep((s) => s + 1);
  };

  // Convert "16:15" → "04:15 PM"
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("payment_method", form.payment); // customer preference — Razorpay is the gateway
      ...
      // Step 2: Create Razorpay payment intent for visiting charge (advance)
        ...
        const options = {
          ...
          handler: async (response: any) => {
            try {
              ...
              await api.post(
                `/api/booking/${bookingId}/payment/razorpay/verify/`,
                {
      ...
      // Step 5: All done — redirect
      router.push("/customer/my-bookings?booked=1");
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      <button
                        key={cat.id}
                        onClick={() => {
                          setForm({ ...form, category: cat.name, category_id: cat.id });
                          setError("");
                        }}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`font-semibold ${selected ? "text-blue-700" : "text-gray-900"}`}>
                            {w.name}
                          </p>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  name="problem_title"
                  placeholder="E.g. Tap leaking, Fan not working"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="E.g. The kitchen tap has been leaking since yesterday..."
          ...
          <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-100">
            ...
            {step < 3 ? (
              <button
                onClick={goNext}
                disabled={step === 1 && !form.category_id}
