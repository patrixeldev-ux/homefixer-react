  ...
  return (
    <main className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-gray-500 text-sm mb-5">All your service bookings in one place</p>
            ...
            {filtered.map(b => (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <span className={`shrink-0 px-2.5 py-0.5 rounded-full text-xs font-bold ${statusStyle[b.status] ?? "bg-gray-100 text-gray-700"}`}>
                    {b.status}
                  </span>
                </div>
                ...
                <div className="space-y-1 text-sm">
                  <p className="text-gray-700 flex gap-2">
                    <span>📍</span><span>{formatAddress(b.customer_address)}</span>
                  </p>
                  <p className="text-gray-700 flex gap-2">
                    <span>📅</span><span>{b.scheduled_date} at {b.scheduled_time}</span>
                  </p>
                  <p className="text-gray-700 flex gap-2">
                    <span>💰</span><span>Total: ₹{b.total_amount}</span>
                  </p>
                </div>
